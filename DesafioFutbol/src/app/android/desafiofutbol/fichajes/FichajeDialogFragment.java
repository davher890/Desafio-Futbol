...
public class FichajeDialogFragment extends DialogFragment {
    ...
    public AlertDialog createDialogLugar(final Context contexto, final View v){
		...
		builder.setTitle(jugador.getApodo())
        	   .setPositiveButton(buttonOferta, new DialogInterface.OnClickListener() {
        		   public void onClick(DialogInterface dialog, int id) {
        			   if (!cantidad.getText().toString().equals("")){
        				   int cantidadOferta = Integer.parseInt(cantidad.getText().toString());
        				   if (cantidadOferta < jugador.getValor()){
        					   Toast.makeText(fragment.getActivity(), "El valor mínimo para fichar a "+jugador.getApodo()+" es "+jugador.getValor(), Toast.LENGTH_SHORT);
        				   }        			   
	        			   else{
		        			   ...
		        			   PostDesafioFutbol post = new PostDesafioFutbol("mercado/create_ofertas", fragment, json, "gestionaWS");
		        			   post.execute();
               ...
               .setNegativeButton("Pagar Cláusula ("+formatterValor.format(2*jugador.getValor())+")", new DialogInterface.OnClickListener() {
        		   public void onClick(DialogInterface dialog, int id) {
        			   JSONObject json = new JSONObject();
                       try {
                    	   json.put("id", jugador.getId());	 
        			   ...
        			   PostDesafioFutbol post = new PostDesafioFutbol("mercado/"+FichajeDialogFragment.this.jugador.getIdMercado()+"/clausula_pagar", fragment, json, "gestionaWS");
        			   post.execute();
                       dialog.cancel();
                   }
               })
               .setNeutralButton("Eliminar Oferta", new DialogInterface.OnClickListener() {
        		   public void onClick(DialogInterface dialog, int id) {
    				   JSONObject json = new JSONObject();
                       try {
